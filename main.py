...
# Builds the Anthropic client which is used to send requests to the Claude API
anthropic_client = Anthropic()  # picks up ANTHROPIC_API_KEY from the environment automatically
...
#PROMPT EXAMPLE ========================
# I'm worried that a device starting with "zeus-corp" might have been maliciously logged into in the last day

# return an object that describes the user's request as well as where and how the agent has decided to search
unformatted_query_context = EXECUTOR.get_query_context(anthropic_client, user_message, model=model)
...
#query_context["time_range_hours"] = min(query_context["time_range_hours"],0.30)  # Limit the time range to 0.30 hours (18 minutes) for testing purposes. This is a temporary measure to avoid long-running queries during development and testing.
...
# Grab the threat hunt system prompt (a plain string for Claude's system= param)
threat_hunt_system_message = PROMPT_MANAGEMENT.SYSTEM_PROMPT_THREAT_HUNT

# Count / estimate total input tokens
threat_hunt_messages = [threat_hunt_user_message]
