...
# NOTE FOR CLAUDE: these are now plain strings, not {"role": "system", ...} dicts.
# Claude's API takes system instructions via a separate `system=` parameter on
# the messages.create() call — they must NOT be placed inside the `messages` list.
...
SYSTEM_PROMPT_THREAT_HUNT = (
    ...
    "You have access to a tool called report_threat_hunt_findings. Use it to return your findings in "
    "structured form once your analysis is complete."
)
...
# Claude-shaped tool definition: flat structure (no "type": "function" wrapper),
# and "input_schema" instead of "parameters".
TOOLS = [
    {
